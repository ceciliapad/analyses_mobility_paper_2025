...
png ("results/reasons_visit_traditional/places_all_regions_live.png", units="in", width=6, height=5.9, res=500)
...
for (i in 1:nrow(dongou_visited)){
  ...
  reason <- str_detect("live", my_reasons)
...
times_live <- vector(length=nrow(visits_dist_enyelle))
for (i in 1:nrow(enyelle_reasons)) {
  my_reasons <- as.character(enyelle_reasons[i,])
  reason <- str_detect("live", my_reasons)
  reasons <- which(reason==TRUE)
  total_live <- length(reasons)
  times_live[i] <- total_live
}
visits_dist_enyelle$total_live <- times_live
...
times_live <- vector(length=nrow(visits_dist_betou))
...
visits_dist_betou$total_live <- times_live
...
times_live <- vector(length=nrow(visits_dist_macao_22))
...
visits_dist_macao_22$total_live <- times_live
...
times_live <- vector(length=nrow(visits_dist_macao_23))
...
visits_dist_macao_23$total_live <- times_live
...
times_live <- vector(length=nrow(visits_dist_minganga_22))
...
visits_dist_minganga_22$total_live <- times_live
...
times_live <- vector(length=nrow(visits_dist_minganga_23))
...
visits_dist_minganga_23$total_live <- times_live
...
write.csv(visits_dist_macao_22, "results/reasons_visit_traditional/visits_distances_macao_22_max_live.csv")
write.csv(visits_dist_macao_23, "results/reasons_visit_traditional/visits_distances_macao_23_max_live.csv")
write.csv(visits_dist_enyelle, "results/reasons_visit_traditional/visits_distances_enyelle_max_live.csv")
write.csv(visits_dist_betou, "results/reasons_visit_traditional/visits_distances_betou_max_live.csv")
write.csv(visits_dist_minganga_22, "results/reasons_visit_traditional/visits_distances_minganga_22_max_live.csv")
write.csv(visits_dist_minganga_23, "results/reasons_visit_traditional/visits_distances_minganga_23_max_live.csv")
...
colnames(visits_dist_macao_22_red) <- c("residence_camp", "ref", "sex", "birthplace", "age", "median_dist",
                                        "max_dist", "min_dist", "places_visited", "times_live", "region")
...
write.csv(visits_dist_all, "results/reasons_visit_traditional/dist_everyone_live.csv")
...
write.csv(sum_df_by_age_sex, "results/reasons_visit_traditional/sum_df_by_age_sex_live.csv")
write.csv(sum_df_by_age, "results/reasons_visit_traditional/sum_df_by_age_live.csv")
...
p1 <- visits_dist_all %>% dplyr::select(median_dist_km, age, sex) %>%
  ...
  labs(x = "Age group", y = "Median distance travelled to live (km)") +
  theme_minimal() +
  ggtitle("live") + 
  geom_boxplot()
...
pi
ggplot2::ggsave("results/reasons_visit_traditional/sex_age_median_from_res_live.png", width = 7, height=6)
...
visits_dist_all %>%
  ...
  ggstatsplot::grouped_ggbetweenstats(x = sex, y = "median_dist_km", 
                                      ...
                                      ylab="Median distance travelled to live (km)",
                                      xlab="Sex",
...
ggplot2::ggsave("results/reasons_visit_traditional/sex_age_median_from_res_stats_live.png", width = 17, height=8)
...
#ggplot2::scale_fill_manual(values=c(MetPalettes$Isfahan1[[1]][4], MetPalettes$Isfahan1[[1]][6]))
ggplot2::ggsave("results/reasons_visit_traditional/age_sex_median_from_res_stats_live.png", width = 11, height=7)

# # READ IN DATA

visits_dist_macao_22 <- read.csv("results/reasons_visit_traditional/visits_distances_macao_22_max_live.csv")[,-c(1:6,8:9)]
visits_dist_macao_23 <- read.csv("results/reasons_visit_traditional/visits_distances_macao_23_max_live.csv")[,-c(1:4, 6:9)]
visits_dist_enyelle <- read.csv("results/reasons_visit_traditional/visits_distances_enyelle_max_live.csv")[,-c(1:6,8:9)]
visits_dist_betou <- read.csv("results/reasons_visit_traditional/visits_distances_betou_max_live.csv")[,-c(1:6,8:9)]
visits_dist_minganga_22 <- read.csv("results/reasons_visit_traditional/visits_distances_minganga_22_max_live.csv")[,-c(1:6,8:9)]
visits_dist_minganga_23 <- read.csv("results/reasons_visit_traditional/visits_distances_minganga_23_max_live.csv")[,-c(1:5, 7:11)]
...
for (region_name in names(region_datasets)) {
  ...
  cols_to_remove <- c("median", "max", "min", "places_visited", "total_live", "Sexe")
  ...
  write.csv(summary_stats, file = paste0("results/reasons_visit_traditional/summary_statistics_", region_name, "_live.csv"), row.names = FALSE)
}
...
write.csv(combined_summary_stats, "results/reasons_visit_traditional/summary_statistics_all_regions_live.csv", row.names = FALSE)




#Make density plots of distances travelled to live
# separate by sex
...
visits_dist_enyelle_m$total_live <- NULL
...
visits_dist_enyelle_f$total_live <- NULL
...
#hist(visits_dist_enyelle/1000, main="Distance travelled to live members")
...
visits_dist_betou_m$total_live <- NULL
...
visits_dist_betou_f$total_live <- NULL
...
visits_dist_macao_22_m$total_live <- NULL
...
visits_dist_macao_22_f$total_live <- NULL
...
visits_dist_macao_23_m$total_live <- NULL
...
visits_dist_macao_23_f$total_live <- NULL
...
visits_dist_minganga_22_m$total_live <- NULL
...
visits_dist_minganga_22_f$total_live <- NULL
...
visits_dist_minganga_23_m$total_live <- NULL
...
visits_dist_minganga_23_f$total_live <- NULL
...
# only one woman liveed
write.csv(summary_df, "results/reasons_visit_traditional/summary_statistics_live.csv", row.names = FALSE)
...
ggplot(graph, aes(x = value, fill = category)) +
  ...
  theme_bw() +
  scale_fill_manual(values=c(MetPalettes$Isfahan1[[1]][4], MetPalettes$Isfahan1[[1]][6])) +
  labs(title="Distances travelled to live",x="Km", y = "Count") +
  ...
  annotate("text", x = mean_vals[1]+60, y = 30, label = round(mean_vals[1], 2),
  ...
  annotate("text", x = mean_vals[2]+60, y = 25, label = round(mean_vals[2], 2),
           color = MetPalettes$Isfahan1[[1]][6], size =5, vjust = -1) + 
  scale_x_continuous(breaks = seq(0, 1000, by = 100), limits = c(0,150))
ggsave("results/reasons_visit_traditional/distance_live_by_sex_200.png", width = 6.5, height=5)
