...
png ("results/reasons_visit/places_all_regions_dance.png", units="in", width=6, height=5.9, res=500)
...
for (i in 1:nrow(dongou_visited)){
  ...
  reason <- str_detect("dance|funeral|ritual|hg_politics|announce_marriage", my_reasons)
...
times_dance <- vector(length=nrow(visits_dist_dongou))
for (i in 1:nrow(dongou_reasons)) {
  my_reasons <- as.character(dongou_reasons[i,])
  reason <- str_detect("dance|funeral|ritual|hg_politics|announce_marriage", my_reasons)
  reasons <- which(reason==TRUE)
  total_dance <- length(reasons)
  times_dance[i] <- total_dance
}
visits_dist_dongou$total_dance <- times_dance
...
times_dance <- vector(length=nrow(visits_dist_enyelle))
...
visits_dist_enyelle$total_dance <- times_dance
...
times_dance <- vector(length=nrow(visits_dist_betou))
...
visits_dist_betou$total_dance <- times_dance
...
times_dance <- vector(length=nrow(visits_dist_macao_22))
...
visits_dist_macao_22$total_dance <- times_dance
...
times_dance <- vector(length=nrow(visits_dist_macao_23))
...
visits_dist_macao_23$total_dance <- times_dance
...
times_dance <- vector(length=nrow(visits_dist_minganga_22))
...
visits_dist_minganga_22$total_dance <- times_dance
...
times_dance <- vector(length=nrow(visits_dist_minganga_23))
...
visits_dist_minganga_23$total_dance <- times_dance
...
write.csv(visits_dist_dongou, "results/reasons_visit/visits_distances_dongou_max_dance.csv")
write.csv(visits_dist_macao_22, "results/reasons_visit/visits_distances_macao_22_max_dance.csv")
write.csv(visits_dist_macao_23, "results/reasons_visit/visits_distances_macao_23_max_dance.csv")
write.csv(visits_dist_enyelle, "results/reasons_visit/visits_distances_enyelle_max_dance.csv")
write.csv(visits_dist_betou, "results/reasons_visit/visits_distances_betou_max_dance.csv")
write.csv(visits_dist_minganga_22, "results/reasons_visit/visits_distances_minganga_22_max_dance.csv")
write.csv(visits_dist_minganga_23, "results/reasons_visit/visits_distances_minganga_23_max_dance.csv")
...
write.csv(visits_dist_all, "results/reasons_visit/dist_everyone_dance.csv")

# READ IN DATA

visits_dist_dongou <- read.csv("results/reasons_visit/visits_distances_dongou_max_dance.csv")[,-c(1:6,8:9)]
visits_dist_macao_22 <- read.csv("results/reasons_visit/visits_distances_macao_22_max_dance.csv")[,-c(1:6,8:9)]
visits_dist_macao_23 <- read.csv("results/reasons_visit/visits_distances_macao_23_max_dance.csv")[,-c(1:4,6:9)]
visits_dist_enyelle <- read.csv("results/reasons_visit/visits_distances_enyelle_max_dance.csv")[,-c(1:6,8:9)]
visits_dist_betou <- read.csv("results/reasons_visit/visits_distances_betou_max_dance.csv")[,-c(1:6,8:9)]
visits_dist_minganga_22 <- read.csv("results/reasons_visit/visits_distances_minganga_22_max_dance.csv")[,-c(1:6,8:9)]
visits_dist_minganga_23 <- read.csv("results/reasons_visit/visits_distances_minganga_23_max_dance.csv")[,-c(1:5, 7:11)]
...
for (region_name in names(region_datasets)) {
  ...
  cols_to_remove <- c("median", "max", "min", "places_visited", "total_dance", "Sexe")
  ...
  write.csv(summary_stats, file = paste0("results/reasons_visit/summary_statistics_", region_name, "_dance.csv"), row.names = FALSE)
}
...
write.csv(combined_summary_stats, "results/reasons_visit/summary_statistics_all_regions_dance.csv", row.names = FALSE)


# Make density plots of distances travelled for rituals
# separate by sex
...
visits_dist_enyelle_m$total_dance <- NULL
...
visits_dist_enyelle_f$total_dance <- NULL
...
visits_dist_betou_m$total_dance <- NULL
...
visits_dist_betou_f$total_dance <- NULL
...
visits_dist_dongou_m$total_dance <- NULL
...
visits_dist_dongou_f$total_dance <- NULL
...
visits_dist_macao_22_m$total_dance <- NULL
...
visits_dist_macao_22_f$total_dance <- NULL
...
visits_dist_macao_23_m$total_dance <- NULL
...
visits_dist_macao_23_f$total_dance <- NULL
...
visits_dist_minganga_22_m$total_dance <- NULL
...
visits_dist_minganga_22_f$total_dance <- NULL
...
visits_dist_minganga_23_m$total_dance <- NULL
...
visits_dist_minganga_23_f$total_dance <- NULL
...
# only one woman danceed
write.csv(summary_df, "results/reasons_visit/summary_statistics_dance.csv", row.names = FALSE)
...
ggplot(graph, aes(x = value, fill = category)) +
  ...
  labs(title="Distances travelled for rituals",x="Km", y = "Count") +
  ...
  annotate("text", x = mean_vals[1]+80, y = 20, label = round(mean_vals[1], 2),
  ...
  annotate("text", x = mean_vals[2]+80, y = 25, label = round(mean_vals[2], 2),
           color = MetPalettes$Isfahan1[[1]][6], size =5, vjust = -1) + 
  scale_x_continuous(breaks = seq(0, 1000, by = 100), limits = c(0,900))
ggsave("results/reasons_visit/distance_dance_by_sex.png", width = 6.5, height=5)
...
write.csv(sum_df_by_age_sex, "results/reasons_visit/sum_df_by_age_sex_dance.csv")
write.csv(sum_df_by_age, "results/reasons_visit/sum_df_by_age_dance.csv")
...
p1 <- visits_dist_all %>% dplyr::select(median_dist_km, age, sex) %>%
  ...
  scale_fill_manual(values=c(MetPalettes$Isfahan1[[1]][4], MetPalettes$Isfahan1[[1]][6])) + 
  labs(x = "Age group", y = "Median distance travelled for rituals (km)") +
  theme_minimal() +
  ggtitle("dance") + 
  geom_boxplot()
...
pi
ggplot2::ggsave("results/reasons_visit/sex_age_median_from_res_dance.png", width = 7, height=6)
...
visits_dist_all %>%
  ...
  ggstatsplot::grouped_ggbetweenstats(x = sex, y = "median_dist_km", 
                                      ...
                                      ylab="Median distance travelled for rituals (km)",
                                      xlab="Sex",
...
ggplot2::ggsave("results/reasons_visit/sex_age_median_from_res_stats_dance.png", width = 17, height=8)
...
#ggplot2::scale_fill_manual(values=c(MetPalettes$Isfahan1[[1]][4], MetPalettes$Isfahan1[[1]][6]))
ggplot2::ggsave("results/reasons_visit/age_sex_median_from_res_stats_dance.png", width = 11, height=7)
