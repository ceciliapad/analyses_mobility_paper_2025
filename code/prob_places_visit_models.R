...
# if overall
data <- read.csv("data/final/places_visited_reasons_proportion_nov_24.csv")
...
write.csv(data_cleaned, "data/final/places_visited_reasons_proportion_nov_24_cleaned.csv")
...
data_cleaned <- data_cleaned %>%
  mutate(across(11:20, as.numeric))

# Define response variables and their corresponding models
response_vars <- c(
  "times_fish", "times_bantu_work", "times_hunt", 
...
for (i in 1:nrow(data_cleaned)) {
  if (data_cleaned$places_visited[i] < sum(data_cleaned[i, 12:20])) {
    print("places less than sum")
    data_cleaned$places_visited[i] <- sum(data_cleaned[i, 12:20])
  }
}
...
formula <- bf(as.formula(paste0(var, " | trials(places_visited) ~ age + sex +
  ...
  prior_model <- brm(
    formula = formula,
    data = data_cleaned,  # Use combined_data structure
  ...
  # Extract prior samples for intercept
  prior_draws <- as_draws_df(prior_model)
  prior_draws$probability <- plogis(prior_draws$b_Intercept)  # Convert to probability scale


ggplot(prior_draws, aes(x = probability)) +
  geom_density(alpha = 0.7) +
  #facet_wrap(~response, scales = "free", ncol = 3) +
  theme_minimal() +
  labs(
    title = "Prior Predictive Distributions (Probability Scale)",
    x = "Probability",
    y = "Density"
  ) +
  theme(legend.position = "none")
# Save plot
ggsave("results/reasons_visit_proportion/prior_predictive_distributions_binomial_activity_proportion.png", width = 6, height = 6)
...
for (var in response_vars) {
  ...
  model <- brm(
    ...
    cores = 4, 
    control = list(adapt_delta = 0.95),
    file = paste0("models/", var, "_model_fit")
  )
...
saveRDS(model_results, file = "models/binomial_models_results_interaction_prop_places_visited_reason.rds")
...
for (var in response_vars) { # we dont need ID random effect as no repeated obs
  ...
  model <- brm(
    ...
    cores = 4, 
    control = list(adapt_delta = 0.95),
    file = paste0("models/", var, "_model_fit_no_int")
  )
...
# Save all models to disk for later inspection
saveRDS(model_results_no_int, file = "models/binomial_models_results_no_interaction_prop_places_visited_reason.rds")
...
for (var in response_vars) { # we dont need ID random effect as no repeated obs
  ...
  model <- brm(
    ...
    cores = 4, 
    control = list(adapt_delta = 0.95),
    file = paste0("models/", var, "_model_intercept")
  )
...
# Save all models to disk for later inspection
saveRDS(model_results_intercept, file = "models/binomial_models_results_interaction_prop_places_visited_reason_randomintercept.rds")
...
model_results <- readRDS("models/binomial_models_results_interaction_prop_places_visited_reason.rds")
model_results_no_int <- readRDS("models/binomial_models_results_no_interaction_prop_places_visited_reason.rds")
model_results_intercept <- readRDS("models/binomial_models_results_interaction_prop_places_visited_reason_randomintercept.rds")
...
write.csv(all_loo_comparisons, "results/reasons_visit_proportion/loo_comparisons_results.csv", row.names = FALSE)
...
region_levels <- c("Dongou", "Macao", "Minganga", "Enyelle", "Betou")
...
# save data frame
write.csv(all_predictions, "results/reasons_visit_proportion/all_predictions.csv")
...
# Save the plot
ggplot2::ggsave("results/reasons_visit/activity_specific_probability_plot_simulated_places_fixed.png", width = 12, height = 15)
...
# save data frame
write.csv(all_predictions, "results/reasons_visit_proportion/all_predictions_averaged_regions.csv")

# Ensure sex is a factor with desired levels
...
# Save the plot
ggplot2::ggsave("results/reasons_visit_proportion/activity_specific_probability_plot_averaged_regions.png", width = 10, height = 8)
...
# Save the plot
ggplot2::ggsave("results/reasons_visit_proportion/activity_specific_probability_plot_averaged_regions_box.png", width = 10, height = 8)
...
all_predictions %>%
  ...
  scale_fill_manual(values = rev(dark_sex_colors)) +
  scale_color_manual(values = rev(dark_sex_colors))

# Save the ribbon plot
ggplot2::ggsave("results/reasons_visit_proportion/activity_specific_probability_ribbon_averaged_age.png", width = 10, height = 8)
...
# Save the bar plot
ggplot2::ggsave("results/reasons_visit_proportion/activity_specific_probability_bar_averaged_age.png", width = 10, height = 8)
...
for (var in response_vars) {
  ...
  model <- brm(
    ...
    cores = 4, 
    control = list(adapt_delta = 0.95),
    file = paste0("models/", var, "_model_fit_adult_old")
  )
...
# Save all models to disk for later inspection
saveRDS(model_results, file = "models/binomial_models_results_interaction_prop_places_visited_reason_adults_old_together.rds")

# Read in the saved model results
model_results <- readRDS("models/binomial_models_results_interaction_prop_places_visited_reason_adults_old_together.rds")
...
# save data frame
write.csv(all_predictions, "results/reasons_visit_proportion/all_predictions_adult_old_merge_regions_separate.csv")
...
# Save the plot
ggplot2::ggsave("results/reasons_visit/activity_specific_probability_plot_simulated_places_fixed_adult_old_merge.png", width = 10, height = 15)
...
# save data frame
write.csv(all_predictions, "results/reasons_visit_proportion/all_predictions_averaged_regions_adult_old_merge.csv")
...
# Save the plot
ggplot2::ggsave("results/reasons_visit_proportion/activity_specific_probability_plot_averaged_regions_adult_old_merge.png", width = 10, height = 8)
...
all_predictions %>%
  ...
  #scale_fill_manual(values = dark_sex_colors) +
  scale_color_manual(values = rev(light_sex_colors))

# Save the plot
ggplot2::ggsave("results/reasons_visit_proportion/activity_specific_probability_plot_averaged_regions_box_adult_old_merge.png", width = 10, height = 8)
