...
png ("results/reasons_visit_traditional/places_all_regions_fish.png", units="in", width=6, height=5.9, res=500)
...
for (i in 1:nrow(macao_visited_22)){
  ...
  reason <- str_detect("fish", my_reasons)
...
times_fish <- vector(length=nrow(visits_dist_enyelle))
for (i in 1:nrow(enyelle_reasons)) {
  my_reasons <- as.character(enyelle_reasons[i,])
  reason <- str_detect("fish", my_reasons)
  reasons <- which(reason==TRUE)
  total_fish <- length(reasons)
  times_fish[i] <- total_fish
}
visits_dist_enyelle$total_fish <- times_fish
...
times_fish <- vector(length=nrow(visits_dist_betou))
...
visits_dist_betou$total_fish <- times_fish
...
times_fish <- vector(length=nrow(visits_dist_macao_22))
...
visits_dist_macao_22$total_fish <- times_fish
...
times_fish <- vector(length=nrow(visits_dist_macao_23))
...
visits_dist_macao_23$total_fish <- times_fish
...
times_fish <- vector(length=nrow(visits_dist_minganga_22))
...
visits_dist_minganga_22$total_fish <- times_fish
...
times_fish <- vector(length=nrow(visits_dist_minganga_23))
...
visits_dist_minganga_23$total_fish <- times_fish
...
write.csv(visits_dist_macao_22, "results/reasons_visit_traditional/visits_distances_macao_22_max_fish.csv")
write.csv(visits_dist_macao_23, "results/reasons_visit_traditional/visits_distances_macao_23_max_fish.csv")
write.csv(visits_dist_enyelle, "results/reasons_visit_traditional/visits_distances_enyelle_max_fish.csv")
write.csv(visits_dist_betou, "results/reasons_visit_traditional/visits_distances_betou_max_fish.csv")
write.csv(visits_dist_minganga_22, "results/reasons_visit_traditional/visits_distances_minganga_22_max_fish.csv")
write.csv(visits_dist_minganga_23, "results/reasons_visit_traditional/visits_distances_minganga_23_max_fish.csv")
...
colnames(visits_dist_macao_22_red) <- c("residence_camp", "ref", "sex", "birthplace", "age", "median_dist",
                                        "max_dist", "min_dist", "places_visited", "times_fish", "region")
...
write.csv(visits_dist_all, "results/reasons_visit_traditional/dist_everyone_fish.csv")
...
write.csv(sum_df_by_age_sex, "results/reasons_visit_traditional/sum_df_by_age_sex_fish.csv")
write.csv(sum_df_by_age, "results/reasons_visit_traditional/sum_df_by_age_fish.csv")
...
p1 <- visits_dist_all %>% dplyr::select(median_dist_km, age, sex) %>%
  ...
  labs(x = "Age group", y = "Median distance travelled to fish (km)") +
  theme_minimal() +
  ggtitle("fish") + 
  geom_boxplot()
...
pi
ggplot2::ggsave("results/reasons_visit_traditional/sex_age_median_from_res_fish.png", width = 7, height=6)
...
visits_dist_all %>%
  ...
  ggstatsplot::grouped_ggbetweenstats(x = sex, y = "median_dist_km", 
                                      ...
                                      ylab="Median distance travelled to fish (km)",
                                      xlab="Sex",
...
ggplot2::ggsave("results/reasons_visit_traditional/sex_age_median_from_res_stats_fish.png", width = 17, height=8)
...
#ggplot2::scale_fill_manual(values=c(MetPalettes$Isfahan1[[1]][4], MetPalettes$Isfahan1[[1]][6]))
ggplot2::ggsave("results/reasons_visit_traditional/age_sex_median_from_res_stats_fish.png", width = 11, height=7)

# # READ IN DATA

visits_dist_macao_22 <- read.csv("results/reasons_visit_traditional/visits_distances_macao_22_max_fish.csv")[,-c(1:6,8:9)]
visits_dist_macao_23 <- read.csv("results/reasons_visit_traditional/visits_distances_macao_23_max_fish.csv")[,-c(1:4, 6:9)]
visits_dist_enyelle <- read.csv("results/reasons_visit_traditional/visits_distances_enyelle_max_fish.csv")[,-c(1:6,8:9)]
visits_dist_betou <- read.csv("results/reasons_visit_traditional/visits_distances_betou_max_fish.csv")[,-c(1:6,8:9)]
visits_dist_minganga_22 <- read.csv("results/reasons_visit_traditional/visits_distances_minganga_22_max_fish.csv")[,-c(1:6,8:9)]
visits_dist_minganga_23 <- read.csv("results/reasons_visit_traditional/visits_distances_minganga_23_max_fish.csv")[,-c(1:5, 7:11)]
...
for (region_name in names(region_datasets)) {
  ...
  cols_to_remove <- c("median", "max", "min", "places_visited", "total_fish", "Sexe")
  ...
  write.csv(summary_stats, file = paste0("results/reasons_visit_traditional/summary_statistics_", region_name, "_fish.csv"), row.names = FALSE)
}
...
write.csv(combined_summary_stats, "results/reasons_visit_traditional/summary_statistics_all_regions_fish.csv", row.names = FALSE)




# Make density plots of distances travelled to fish
# separate by sex
...
visits_dist_enyelle_m$total_fish <- NULL
...
visits_dist_enyelle_f$total_fish <- NULL
...
#hist(visits_dist_enyelle/1000, main="Distance travelled to fish members")
...
visits_dist_betou_m$total_fish <- NULL
...
visits_dist_betou_f$total_fish <- NULL
...
visits_dist_dongou_m$total_fish <- NULL
...
visits_dist_dongou_f$total_fish <- NULL
...
visits_dist_macao_22_m$total_fish <- NULL
...
visits_dist_macao_22_f$total_fish <- NULL
...
visits_dist_macao_23_m$total_fish <- NULL
...
visits_dist_macao_23_f$total_fish <- NULL
...
visits_dist_minganga_22_m$total_fish <- NULL
...
visits_dist_minganga_22_f$total_fish <- NULL
...
visits_dist_minganga_23_m$total_fish <- NULL
...
visits_dist_minganga_23_f$total_fish <- NULL
...
# only one woman fished
write.csv(summary_df, "results/reasons_visit_traditional/summary_statistics_fish.csv", row.names = FALSE)
...
ggplot(graph, aes(x = value, fill = category)) +
  ...
  theme_bw() +
  scale_fill_manual(values=c(MetPalettes$Isfahan1[[1]][4], MetPalettes$Isfahan1[[1]][6])) +
  labs(title="Distances travelled to fish",x="Km", y = "Count") +
...
ggsave("results/reasons_visit_traditional/distance_fish_by_sex.png", width = 6.5, height=5)
