...
png ("results/reasons_visit/places_all_regions_fam_visit.png", units="in", width=6, height=5.9, res=500)
...
reason <- str_detect("fam_visit|fam_accomp|fam_help|affinal_visit", my_reasons)
...
for (i in 1:nrow(macao_visited_23)){
  ...
  res <- which(regions$region==my_residence)[1]
  #bp <- which(regions$region==my_bp)
  # location of camp of residence
  res_lat <- as.numeric(regions[res,16])
  res_long <- as.numeric(regions[res,15])
...
times_fam_visit <- vector(length=nrow(visits_dist_dongou))
for (i in 1:nrow(dongou_reasons)) {
  my_reasons <- as.character(dongou_reasons[i,])
reason <- str_detect("fam_visit|fam_accomp|fam_help|affinal_visit", my_reasons)
  reasons <- which(reason==TRUE)
  total_fam_visit <- length(reasons)
  times_fam_visit[i] <- total_fam_visit
}
visits_dist_dongou$total_fam_visit <- times_fam_visit
...
times_fam_visit <- vector(length=nrow(visits_dist_enyelle))
...
visits_dist_enyelle$total_fam_visit <- times_fam_visit
...
times_fam_visit <- vector(length=nrow(visits_dist_betou))
...
visits_dist_betou$total_fam_visit <- times_fam_visit
...
times_fam_visit <- vector(length=nrow(visits_dist_macao_22))
...
visits_dist_macao_22$total_fam_visit <- times_fam_visit
...
times_fam_visit <- vector(length=nrow(visits_dist_macao_23))
...
visits_dist_macao_23$total_fam_visit <- times_fam_visit
...
times_fam_visit <- vector(length=nrow(visits_dist_minganga_22))
...
visits_dist_minganga_22$total_fam_visit <- times_fam_visit
...
times_fam_visit <- vector(length=nrow(visits_dist_minganga_23))
...
visits_dist_minganga_23$total_fam_visit <- times_fam_visit
...
write.csv(visits_dist_dongou, "results/reasons_visit/visits_distances_dongou_max_fam_visit.csv")
write.csv(visits_dist_macao_22, "results/reasons_visit/visits_distances_macao_22_max_fam_visit.csv")
write.csv(visits_dist_macao_23, "results/reasons_visit/visits_distances_macao_23_max_fam_visit.csv")
write.csv(visits_dist_enyelle, "results/reasons_visit/visits_distances_enyelle_max_fam_visit.csv")
write.csv(visits_dist_betou, "results/reasons_visit/visits_distances_betou_max_fam_visit.csv")
write.csv(visits_dist_minganga_22, "results/reasons_visit/visits_distances_minganga_22_max_fam_visit.csv")
write.csv(visits_dist_minganga_23, "results/reasons_visit/visits_distances_minganga_23_max_fam_visit.csv")
...
write.csv(visits_dist_all, "results/reasons_visit/dist_everyone_fam_visit.csv")

# READ IN DATA

visits_dist_dongou <- read.csv("results/reasons_visit/visits_distances_dongou_max_fam_visit.csv")[,-c(1:6,8:9)]
visits_dist_macao_22 <- read.csv("results/reasons_visit/visits_distances_macao_22_max_fam_visit.csv")[,-c(1:6,8:9)]
visits_dist_macao_23 <- read.csv("results/reasons_visit/visits_distances_macao_23_max_fam_visit.csv")[,-c(1:4,6:9)]
visits_dist_enyelle <- read.csv("results/reasons_visit/visits_distances_enyelle_max_fam_visit.csv")[,-c(1:6,8:9)]
visits_dist_betou <- read.csv("results/reasons_visit/visits_distances_betou_max_fam_visit.csv")[,-c(1:6,8:9)]
visits_dist_minganga_22 <- read.csv("results/reasons_visit/visits_distances_minganga_22_max_fam_visit.csv")[,-c(1:6,8:9)]
visits_dist_minganga_23 <- read.csv("results/reasons_visit/visits_distances_minganga_23_max_fam_visit.csv")[,-c(1:5, 7:11)]
...
for (region_name in names(region_datasets)) {
  ...
  cols_to_remove <- c("median", "max", "min", "places_visited", "total_fam_visit", "Sexe")
  ...
  write.csv(summary_stats, file = paste0("results/reasons_visit/summary_statistics_", region_name, "_fam_visit.csv"), row.names = FALSE)
}
...
write.csv(combined_summary_stats, "results/reasons_visit/summary_statistics_all_regions_fam_visit.csv", row.names = FALSE)


# Make density plots of distances travelled to visit family
# separate by sex
...
visits_dist_enyelle_m$total_fam_visit <- NULL
...
visits_dist_enyelle_f$total_fam_visit <- NULL
...
visits_dist_betou_m$total_fam_visit <- NULL
...
visits_dist_betou_f$total_fam_visit <- NULL
...
visits_dist_dongou_m$total_fam_visit <- NULL
...
visits_dist_dongou_f$total_fam_visit <- NULL
...
visits_dist_macao_22_m$total_fam_visit <- NULL
...
visits_dist_macao_22_f$total_fam_visit <- NULL
...
visits_dist_macao_23_m$total_fam_visit <- NULL
...
visits_dist_macao_23_f$total_fam_visit <- NULL
...
visits_dist_minganga_22_m$total_fam_visit <- NULL
...
visits_dist_minganga_22_f$total_fam_visit <- NULL
...
visits_dist_minganga_23_m$total_fam_visit <- NULL
...
visits_dist_minganga_23_f$total_fam_visit <- NULL
...
# only one woman fam_visited
write.csv(summary_df, "results/reasons_visit/summary_statistics_fam_visit.csv", row.names = FALSE)
...
ggplot(graph, aes(x = value, fill = category)) +
  ...
  labs(title="Distances travelled to visit family",x="Km", y = "Count") +
  ...
  annotate("text", x = mean_vals[1]+75, y = 30, label = round(mean_vals[1], 2),
  ...
  annotate("text", x = mean_vals[2]+72, y = 25, label = round(mean_vals[2], 2),
           color = MetPalettes$Isfahan1[[1]][6], size =5, vjust = -1) + 
  scale_x_continuous(breaks = seq(0, 1000, by = 100), limits = c(0,300))
ggsave("results/reasons_visit/distance_fam_visit_by_sex_200.png", width = 6.5, height=5)
...
write.csv(sum_df_by_age_sex, "results/reasons_visit/sum_df_by_age_sex_fam_visit.csv")
write.csv(sum_df_by_age, "results/reasons_visit/sum_df_by_age_fam_visit.csv")
...
write.csv(sum_df_by_age_sex_reg, "results/reasons_visit/sum_df_by_sex_age_reg_fam_visit.csv")
...
p1 <- visits_dist_all %>% dplyr::select(median_dist_km, age, sex) %>%
  ...
  scale_fill_manual(values=c( MetPalettes$Isfahan1[[1]][4], MetPalettes$Isfahan1[[1]][6])) + 
  labs(x = "Age group", y = "Median distance travelled to visit family (km)") +
  theme_minimal() +
  ggtitle("Visit family") + 
  geom_boxplot()

# 99% of observations plotted
pi <- p1 + coord_cartesian(ylim = quantile(visits_dist_all$median_dist_km, c(0.01, 0.95), na.rm=T), clip="on")
pi
ggplot2::ggsave("results/reasons_visit/sex_age_median_from_res_fam_visit.png", width = 7, height=6)
...
ggplot2::ggsave("results/reasons_visit/sex_age_boxplot_with_points_by_region_fam_visit.png", plot = p_region_boxplot, width = 9, height = 11)
...
# Save the faceted boxplot
ggplot2::ggsave("results/reasons_visit/sex_age_boxplot_with_points_by_region_macmin_fam_visit.png", plot = p_region_boxplot_macmin, width = 9, height = 6)
...
write.csv(results, "results/reasons_visit/wilcoxon_results_sex_differences_fam_visit.csv", row.names = FALSE)
...
write.csv(results, "results/reasons_visit/wilcoxon_results_sex_differences_fam_visit_allages.csv", row.names = FALSE)
...
visits_dist_all %>%
  ...
  ggstatsplot::grouped_ggbetweenstats(x = sex, y = "median_dist_km", 
                                      ...
                                      ylab="Median distance travelled to visit family (km)",
                                      xlab="Sex",
...
ggplot2::ggsave("results/reasons_visit/sex_age_median_from_res_stats_fam_visit.png", width = 17, height=8)
...
#ggplot2::scale_fill_manual(values=c(MetPalettes$Isfahan1[[1]][4], MetPalettes$Isfahan1[[1]][6]))
ggplot2::ggsave("results/reasons_visit/age_sex_median_from_res_stats_fam_visit.png", width = 11, height=7)
