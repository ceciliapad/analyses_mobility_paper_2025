...
file_directory <- "/results/reasons_visit/distances_all_activities"
...
for (file in file_list) {
  ...
  region <- ifelse(grepl("_dongou", file), "dongou",
                   ifelse(grepl("_betou", file), "betou",
...
write.csv(combined_data, file = "results/reasons_visit/distances_all_activities.csv", row.names = FALSE)

# read combined_data
combined_data <- read.csv("results/reasons_visit/distances_all_activities.csv")
## Prepare models

# Gamma distribution because distance is positive and right-skewed
# 2596 displacements for one of those 9 activities
...
# Make some plots
intercept_prior <- rt(1000, df = 3) * 2 + 3.5
...
ggplot2::ggsave("results/reasons_visit/prior_dist.png", width = 8, height = 7)
...
# Save the models
saveRDS(m1, file = "models/distances_all_activities_m1.rds")
saveRDS(m2, file = "models/distances_all_activities_m2.rds")
saveRDS(m3, file = "models/distances_all_activities_m3.rds")
saveRDS(m4, file = "models/distances_all_activities_m4.rds")

# load the models
m1 <- readRDS("models/distances_all_activities_m1.rds")
m2 <- readRDS("models/distances_all_activities_m2.rds")
m3 <- readRDS("models/distances_all_activities_m3.rds")
m4 <- readRDS("models/distances_all_activities_m4.rds")
...
write.csv(comparison, file = "results/reasons_visit/loo_comparison_reasons_visit.csv", row.names = FALSE)
...
write.csv(posterior_predictions, file = "results/reasons_visit/posterior_predictions_distance_visit_random_slopes.csv", row.names = FALSE)
...
# Save the plot
ggplot2::ggsave("results/reasons_visit/predicted_distance_panel.png", width = 12, height = 10)
...
# Save the plot
ggplot2::ggsave("results/reasons_visit/predicted_distance_barplot_equal_rows.png", width = 12, height = 15)
...
# Save the plot
ggplot2::ggsave("results/reasons_visit/predicted_distance_barplot_averaged_age.png", width = 10, height = 8)

# save data frame
write.csv(posterior_predictions_averaged, file = "results/reasons_visit/posterior_predictions_distance_visit_averaged_age.csv", row.names = FALSE)

# Now we exclude all random effects (but we remove for those activities for which)
# one sex doesn't participate

# Step 1: Check levels of key variables in the original data
combined_data <- combined_data %>%
  mutate(
    age = factor(age, levels = c("Adolescent", "Adulte", "Personne âgée")),
    sex = factor(sex, levels = c("M", "F")),  # Ensure "M" and "F" match original data
    activity = factor(activity, levels = unique(activity))  # Ensure correct levels
  )

# Step 2: Prepare `newdata` for predictions
newdata <- expand.grid(
  age = levels(combined_data$age),
  sex = levels(combined_data$sex),
  activity = levels(combined_data$activity)
)

# Step 3: Filter `newdata` to include only valid groups
valid_groups <- combined_data %>%
  group_by(activity, age, sex) %>%
  summarise(n = n(), .groups = "drop") %>%
  filter(n >= 2)  # Groups with at least 2 observations

newdata <- newdata %>%
  inner_join(valid_groups, by = c("activity", "age", "sex"))

# Step 4: Run the `fitted` function
posterior_predictions <- fitted(
  m2,
  newdata = newdata,
...
ylim(0, 170)  # Set y-axis limits

# Save the plot
ggplot2::ggsave("results/reasons_visit/predicted_distance_avg_regions_panel.png", width = 10, height = 8)

# save data frame
write.csv(posterior_predictions_filtered, file = "results/reasons_visit/posterior_predictions_distance_visit_avg_regions.csv", row.names = FALSE)
...
# Save the plot
ggplot2::ggsave("results/reasons_visit/predicted_distance_dotplot_two_intervals_corrected.png", width = 9, height = 8)

# save data frame
write.csv(posterior_predictions_averaged, file = "results/reasons_visit/posterior_predictions_distance_visit_avg_regions_avg_age.csv", row.names = FALSE)
