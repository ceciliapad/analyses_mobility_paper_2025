...
png ("results/reasons_visit/places_all_regions_bantu_work.png", units="in", width=6, height=5.9, res=500)
...
for (i in 1:nrow(dongou_visited)){
  ...
  reason <- str_detect("bantu_work|work_unsp|bantu_sell|bantu_travel", my_reasons)
...
for (i in 1:nrow(macao_visited_22)){
  ...
  #my_bp <- cbind(bp_long, bp_lat)
  my_reasons <- as.character(my_reasons[1,])
  reason <- str_detect("bantu_work|work_unsp|bantu_sell|bantu_travel", my_reasons) # broadened def on 29.11.2024 to include bantu_sell and travel
  reasons <- which(reason==TRUE)
  #visited <- which(my_places=="Visité?" | my_places=="1")
  # change line below to my_bp if you want to do from BP
...
for (i in 1:nrow(macao_visited_23)){
  ...
  my_pos <- cbind(res_long, res_lat)
  #print(my_pos)
  ifelse(res_lat>4, print(paste(place, "WHAT IS THIS", sep="")),print("."))
  # location of birthplace
  #bp_lat <- as.numeric(regions[bp,16])
  #bp_long <- as.numeric(regions[bp,15])
  #my_bp <- cbind(bp_long, bp_lat)
  ...
  if (length(reasons > 0)) {
    for (j in 1:length(reasons)) {
      ...
      if (length(place) > 0 & !is.na(place)) {
        ...
        visited_point_long <- as.numeric(regions[place,15])
        visited_point_lat <- as.numeric(regions[place,16])
        #ifelse(visited_point_lat>4, print(paste(place, "WHAT IS THIS", sep="")),print("."))
        visited_point <- cbind(visited_point_long, visited_point_lat)
        points(visited_point, pch=16, col=MetPalettes$Isfahan1[[1]][5], cex=0.5)
        #print(visited_point)
        arc <- geosphere::gcIntermediate( c(visited_point[1], visited_point[2]),
...
times_bantu_work <- vector(length=nrow(visits_dist_dongou))
for (i in 1:nrow(dongou_reasons)) {
  my_reasons <- as.character(dongou_reasons[i,])
  reason <- str_detect("bantu_work|work_unsp|bantu_sell|bantu_travel", my_reasons)
  reasons <- which(reason==TRUE)
  total_bantu_work <- length(reasons)
  times_bantu_work[i] <- total_bantu_work
}
visits_dist_dongou$total_bantu_work <- times_bantu_work
...
times_bantu_work <- vector(length=nrow(visits_dist_enyelle))
...
visits_dist_enyelle$total_bantu_work <- times_bantu_work
...
times_bantu_work <- vector(length=nrow(visits_dist_betou))
...
visits_dist_betou$total_bantu_work <- times_bantu_work
...
times_bantu_work <- vector(length=nrow(visits_dist_macao_22))
...
visits_dist_macao_22$total_bantu_work <- times_bantu_work
...
times_bantu_work <- vector(length=nrow(visits_dist_macao_23))
...
visits_dist_macao_23$total_bantu_work <- times_bantu_work
...
times_bantu_work <- vector(length=nrow(visits_dist_minganga_22))
...
visits_dist_minganga_22$total_bantu_work <- times_bantu_work
...
times_bantu_work <- vector(length=nrow(visits_dist_minganga_23))
...
visits_dist_minganga_23$total_bantu_work <- times_bantu_work
...
write.csv(visits_dist_dongou, "results/reasons_visit/visits_distances_dongou_max_bantu_work.csv")
write.csv(visits_dist_macao_22, "results/reasons_visit/visits_distances_macao_22_max_bantu_work.csv")
write.csv(visits_dist_macao_23, "results/reasons_visit/visits_distances_macao_23_max_bantu_work.csv")
write.csv(visits_dist_enyelle, "results/reasons_visit/visits_distances_enyelle_max_bantu_work.csv")
write.csv(visits_dist_betou, "results/reasons_visit/visits_distances_betou_max_bantu_work.csv")
write.csv(visits_dist_minganga_22, "results/reasons_visit/visits_distances_minganga_22_max_bantu_work.csv")
write.csv(visits_dist_minganga_23, "results/reasons_visit/visits_distances_minganga_23_max_bantu_work.csv")
...
write.csv(visits_dist_all, "results/reasons_visit/dist_everyone_bantu_work.csv")


# READ IN DATA

visits_dist_dongou <- read.csv("results/reasons_visit/visits_distances_dongou_max_bantu_work.csv")[,-c(1:6,8:9)]
visits_dist_macao_22 <- read.csv("results/reasons_visit/visits_distances_macao_22_max_bantu_work.csv")[,-c(1:6,8:9)]
visits_dist_macao_23 <- read.csv("results/reasons_visit/visits_distances_macao_23_max_bantu_work.csv")[,-c(1:4,6:9)]
visits_dist_enyelle <- read.csv("results/reasons_visit/visits_distances_enyelle_max_bantu_work.csv")[,-c(1:6,8:9)]
visits_dist_betou <- read.csv("results/reasons_visit/visits_distances_betou_max_bantu_work.csv")[,-c(1:6,8:9)]
visits_dist_minganga_22 <- read.csv("results/reasons_visit/visits_distances_minganga_22_max_bantu_work.csv")[,-c(1:6,8:9)]
visits_dist_minganga_23 <- read.csv("results/reasons_visit/visits_distances_minganga_23_max_bantu_work.csv")[,-c(1:5, 7:11)]
...
for (region_name in names(region_datasets)) {
  ...
  cols_to_remove <- c("median", "max", "min", "places_visited", "total_bantu_work", "Sexe")
  ...
  write.csv(summary_stats, file = paste0("results/reasons_visit/summary_statistics_", region_name, "_bantu_work.csv"), row.names = FALSE)
}
...
write.csv(combined_summary_stats, "results/reasons_visit/summary_statistics_all_regions_bantu_work.csv", row.names = FALSE)


# Make density plots of distances travelled to bantu_work
# separate by sex
...
visits_dist_enyelle_m$total_bantu_work <- NULL
...
visits_dist_enyelle_f$total_bantu_work <- NULL
...
visits_dist_betou_m$total_bantu_work <- NULL
...
visits_dist_betou_f$total_bantu_work <- NULL
...
visits_dist_dongou_m$total_bantu_work <- NULL
...
visits_dist_dongou_f$total_bantu_work <- NULL
...
visits_dist_macao_22_m$total_bantu_work <- NULL
...
visits_dist_macao_22_f$total_bantu_work <- NULL
...
visits_dist_macao_23_m$total_bantu_work <- NULL
...
visits_dist_macao_23_f$total_bantu_work <- NULL
...
visits_dist_minganga_22_m$total_bantu_work <- NULL
...
visits_dist_minganga_22_f$total_bantu_work <- NULL
...
visits_dist_minganga_23_m$total_bantu_work <- NULL
...
visits_dist_minganga_23_f$total_bantu_work <- NULL
...
# only one woman bantu_worked
write.csv(summary_df, "results/reasons_visit/summary_statistics_bantu_work.csv", row.names = FALSE)
...
ggplot(graph, aes(x = value, fill = category)) +
  ...
  labs(title="Distances travelled for wage labour",x="Km", y = "Count") +
  ...
  annotate("text", x = mean_vals[1]+130, y = 30, label = round(mean_vals[1], 2),
  ...
  annotate("text", x = mean_vals[2]+100, y = 25, label = round(mean_vals[2], 2),
           color = MetPalettes$Isfahan1[[1]][6], size =5, vjust = -1) + 
  scale_x_continuous(breaks = seq(0, 1000, by = 100))
ggsave("results/reasons_visit/distance_bantu_work_by_sex_200.png", width = 6.5, height=5)
...
write.csv(sum_df_by_age_sex, "results/reasons_visit/sum_df_by_age_sex_bantu_work.csv")
write.csv(sum_df_by_age, "results/reasons_visit/sum_df_by_age_bantu_work.csv")
...
p1 <- visits_dist_all %>% dplyr::select(median_dist_km, age, sex) %>%
  ...
  scale_fill_manual(values=c(MetPalettes$Isfahan1[[1]][4], MetPalettes$Isfahan1[[1]][6])) + 
  labs(x = "Age group", y = "Median distance travelled for wage labour (km)") +
  theme_minimal() +
  ggtitle("Wage labour") + 
  geom_boxplot()
...
pi
ggplot2::ggsave("results/reasons_visit/sex_age_median_from_res_bantu_work.png", width = 7, height=6)
...
visits_dist_all %>%
  ...
  ggstatsplot::grouped_ggbetweenstats(x = sex, y = "median_dist_km", 
                                      ...
                                      ylab="Median distance travelled for wage labour(km)",
                                      xlab="Sex",
...
ggplot2::ggsave("results/reasons_visit/sex_age_median_from_res_stats_bantu_work.png", width = 17, height=8)
...
#ggplot2::scale_fill_manual(values=c(MetPalettes$Isfahan1[[1]][4], MetPalettes$Isfahan1[[1]][6]))
ggplot2::ggsave("results/reasons_visit/age_sex_median_from_res_stats_bantu_work.png", width = 11, height=7)
